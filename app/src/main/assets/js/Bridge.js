...
$(function () {
    window.ssReader = Class({
        ...
        highlightSelectionWithNote: function (color, note) {
            try {
                this.highlighter.highlightSelection(color, null);
                var range = window.getSelection().toString();
                var params = {content: range, rangy: this.getHighlights(), color: color, note: note};
                this.clearSelection();
                Highlight.onReceiveHighlightsWithNote(JSON.stringify(params));
            } catch (err) {
                console.log("highlightSelectionwithnote : " + err);
            }
        },
        ...
        getCurrentPageWordCount: function(eventOrigin) {
            this.getCurrentPageWordCount(eventOrigin);
        }
    });
...
function computeLastReadCfi() {
    ...
    FolioPageFragment.storeLastReadCfi(cfi);
       ...
       /**
        * We disable this constructDOMRect function because it's getting uncaught Reference error in below 8 version mobiles
        * and we replace this function from the same foilioreder library issues
        */
    ...
    //this is the function copied from library issues
    function constructDOMRect(rectJsonString) {
    var rectJson = JSON.parse(rectJsonString);
...
var lastFoundVisibleNode;

function getCurrentPageWordCount(eventOrigin){
    viewportRect = constructDOMRect(FolioWebView.getViewportRect(DisplayUnit.CSS_PX));
//    console.log("word count for current page: "+getWordCountFromVisibleDOM());

    //get word count
    getWordCountFromVisibleDOM();

    //compute last read cfi
    viewportRect = constructDOMRect(FolioWebView.getViewportRect(DisplayUnit.CSS_PX));
    var node = getFirstVisibleNode(document.body) || document.body;
    ...
    var formattedDate = date.join("-") + " " + time.join(":");

    FolioPageFragment.currentPageWordCountEvent(lastFoundVisibleNode, eventOrigin, formattedDate);
    //reset
    lastFoundVisibleNode = 0;
    viewportRect = null
}

function getWordCountFromVisibleDOM(){
...
//    console.log(node.length);
    for(var i=0;i<node.length; i++){
        ...
        var range = document.createRange();
        range.selectNode(cn);
//        console.log("getWordCountForVisibleDOM");
//        console.log(cn);
        var rect = RangeFix.getBoundingClientRect(range);
        if(rect == null)
            continue;
        ...
        if((contains || intersects) && cn.childNodes.length > 0){
            findAllTagsInViewPort(cn);
        }
    }

    return lastFoundVisibleNode;
}

function findAllTagsInViewPort(node){
    if(node.childNodes.length == 0){
        if(node.parentNode.tagName.toLowerCase() == 'pl'){
            var pna = node.parentNode.attributes;
            lastFoundVisibleNode = pna[0].nodeValue;
         }
    }
    else {
